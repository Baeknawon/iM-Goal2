...
import { personaDefs, missionDefs } from '../data/personas';
import { calculateFcps } from './fcpsScore';
import { recommendMissionDuration } from './missionDuration';
import { recoveryCriteria, recoveryPlan } from './recoveryFlow';
...
export function answerAssistant(question: string, state: AppState): AssistantReply {
  ...
  const p = personaDefs[state.persona];
  const mission = missionDefs[state.persona];
  const go = (text: string, label: string, to: string): AssistantReply => ({ text, action: { label, to } });
  if (/이앱|어떻게써|사용방법/.test(q)) return go('목표와 기간을 정하고 저축 계획을 확인해요. 소비가 계획에서 벗어나면 회복 미션을 선택하고, 판정 기록과 목표 회복 예상치를 확인할 수 있어요. 보증금은 선택 사항이며 모든 기능은 현재 데모 데이터로 동작해요.', state.hasGoal ? '내 목표부터 보기' : '목표부터 만들기', state.hasGoal ? '/home' : '/chat');
  if (/초기화|대화삭제/.test(q)) return { text: '채팅창 위의 ‘대화 지우기’를 누르면 현재 상황의 대화만 지울 수 있어요. 목표와 미션 기록은 유지됩니다.' };
  if (/급여|분배|입금카드/.test(q)) return go('급여 입금 카드는 미션 성공 후 급여 분배 전인 데모 단계에서 표시돼요. 실제 입금을 감지한 결과는 아니에요. 분배 화면에서 금액과 계획을 먼저 확인할 수 있어요.', '급여 분배 확인', '/salary');
  if (/포기|중단|그만|취소/.test(q)) return state.missionOn
    ? go('미션을 중단해도 예치한 보증금은 전액 반환돼요. 현재 데모에서는 FCPS −5점으로 기록됩니다. 진행 화면에서 ‘중단하기’를 누르고 내용을 확인한 뒤 결정하세요.', '진행 중인 미션 보기', '/missionLive')
    : go('현재 진행 중인 미션은 없어요. 새 미션의 기간과 보증금을 확인한 뒤 시작할 수 있어요.', '미션 목록 보기', '/missions');
  if (/신용점수|공식|대출/.test(q)) return go('FCPS는 예산 준수·미션 같은 금융 행동을 기록하는 데모 점수예요. 공식 신용점수를 대체하지 않으며, 대출 승인이나 금리를 보장하지 않아요.', 'FCPS 상세 확인', '/fcps');
  if (/점수|fcps|마일리지|실패/.test(q)) {
    const score = calculateFcps(state.fcpsLog);
    return go(`현재 FCPS는 ${score.total}점이에요. 시작 500점에 과거 목데이터 적립 112점, 이번 시연 변동 ${score.change >= 0 ? '+' : ''}${score.change}점을 합쳤어요. 미션 성공 +18점, 실패 −8점, 중단 −5점이며 보증금 액수는 점수에 영향을 주지 않아요.`, '점수가 쌓인 기록 보기', '/fcps');
  }
  ...
  if (/예산|소비|지출|잔액/.test(q)) {
    const remaining = p.dailyBudget - state.spent;
    return go(`현재 시연 상태에서 하루 예산은 ${p.dailyBudget.toLocaleString()}원, 오늘 지출은 ${state.spent.toLocaleString()}원이에요. ${remaining >= 0 ? `${remaining.toLocaleString()}원 남았어요.` : `${Math.abs(remaining).toLocaleString()}원 초과했어요.`} 소비분석의 월별 예시 데이터와 시연 트리거 지출은 별도 데이터입니다.`, '소비분석 보기', '/spend');
  }
  ...
  if (/기준|판정|성공|인증/.test(q)) return go(`${mission.title1} ${mission.title2}의 완료 기준은 ‘${recoveryCriteria[state.persona].rule}’예요. ${recoveryCriteria[state.persona].source}를 연결하는 기획이며, 현재는 목데이터 판정 근거를 확인한 뒤 결과를 확정하는 데모입니다.`, '미션 기준 확인', state.missionOn ? '/missionLive' : '/missionDetail');
  if (/도움|회복|얼마나|절약|도착/.test(q)) {
    const plan = state.missionOn && state.activeRecoveryPlan ? state.activeRecoveryPlan : recoveryPlan(state.persona, state.missionDays, state.spent);
    return go(`${plan.missionDays}일 실천 시 절약 예상액은 ${plan.savings.toLocaleString()}원이고, 전액 목표 저축으로 이어가면 ${plan.recoverDays}일 회복을 예상해요. 월 목표 저축액의 하루분으로 환산한 데모 추정이며, 보증금 반환은 절약액에 포함하지 않아요.`, '회복 계획 확인', state.missionOn ? '/missionLive' : '/missionDetail');
  }
  ...
  return { text: '저는 현재 앱의 목표·예산·미션·보증금·FCPS를 안내하는 데모 똑디예요. 자유로운 AI 답변은 아직 연결되지 않았어요. “왜 이 기간을 추천했어?”처럼 앱 기능을 구체적으로 물어보거나 아래 추천 질문을 선택해 주세요.' };
}
