...
export function answerAssistant(question: string, state: AppState): AssistantReply {
  ...
  if (/급여|분배|입금카드/.test(q)) return go('급여 입금 카드는 미션 성공 후 급여 분배 전인 단계에서 표시돼요. 실제 입금을 감지한 결과는 아니에요. 분배 화면에서 금액과 계획을 먼저 확인할 수 있어요.', '급여 분배 확인', '/salary');
  ...
  if (/예산|소비|지출|잔액/.test(q)) {
    ...
    return go(`현재 화면 기준으로 하루 예산은 ${p.dailyBudget.toLocaleString()}원, 오늘 지출은 ${state.spent.toLocaleString()}원이에요. ${remaining >= 0 ? `${remaining.toLocaleString()}원 남았어요.` : `${Math.abs(remaining).toLocaleString()}원 초과했어요.`} 소비분석의 월별 예시 데이터와 현재 위험 알림의 지출은 별도 데이터입니다.`, '소비분석 보기', '/spend');
  }
  ...
  if (/기준|판정|성공|인증/.test(q)) return go(`${mission.title1} ${mission.title2}의 완료 기준은 ‘${recoveryCriteria[state.persona].rule}’예요. ${recoveryCriteria[state.persona].source}를 연결하는 기획이며, 현재는 판정 근거를 확인한 뒤 결과를 확정합니다.`, '미션 기준 확인', state.missionOn ? '/missionLive' : '/missionDetail');
