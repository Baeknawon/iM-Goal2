...
export function SalaryScreen(){
 ...
 const record=s.salaryLog.find(r=>r.id===s.persona+'-'+INITIAL_SPEND_MONTH);
 ...
 {!record&&<CtaButton disabled={s.incomeMonthly<=0} onClick={()=>s.setFueled(true)}>이 분배를 기록에 반영하기</CtaButton>}
 <CtaButton onClick={()=>navigate('/splitSettings')}>{record?'다음 분배 비율 설정':'비율 직접 조정하기'}</CtaButton><CtaButton onClick={()=>navigate('/history')}>저축·미션 기록 보기</CtaButton>
