...
export function VerifyScreen() {
  ...
  const criteria = recoveryCriteria[state.persona];
  if (!state.missionOn) return <Navigate to={state.missionResult ? '/release' : '/missionDetail'} replace />;
  return <Screen>
    <ScreenHeader onBack={() => navigate('/missionLive')} sub="행동 기록을 확인해요" title="미션 판정 근거" />
    <ScreenBody padBottom={24}><div className="screen-stack">
      <Card>
        ...
        <div className="recovery-note"><b>판정 기준</b><p>{criteria.rule}</p><b>확인한 기록 · 목데이터</b><p>{result === 'give_up' ? '사용자 중단 요청 · 미완료 상태로 기록됩니다.' : criteria[result]}</p><b>연결 예정 데이터</b><p>{criteria.source}</p></div>
        <InfoNote>기간 종료 시점의 기록을 가정한 데모 판정입니다. 실제 거래를 조회하거나 실제 수행 기간이 지났다고 판단한 결과는 아닙니다.</InfoNote>
      </Card>
      {result === 'success' && state.activeRecoveryPlan && <RecoveryPlanCard plan={state.activeRecoveryPlan} />}
      <Card><h2 className="fcps-section-title">확정하면 이렇게 반영돼요</h2><p className="fcps-description">보증금 {state.deposit.toLocaleString()}원 전액 반환 · FCPS {result === 'success' ? '+18' : result === 'fail' ? '−8' : '−5'}점. 결과와 날짜가 미션 이력에 남습니다.</p></Card>
    </div></ScreenBody>
    <div className="screen-action-footer"><CtaButton onClick={() => { state.finishMission(result); navigate('/release', { replace: true }); }}>{result === 'give_up' ? '중단 확정 · 보증금 반환' : '판정 확인 · 결과 반영'}</CtaButton></div>
  </Screen>;
}
