...
for(const persona of ['A','B','C'])for(const result of ['fail','give_up'])for(const reason of ['difficulty','time','expense','unknown']){
 s().setPersona(persona);const original=currentMission(s());s().startMission(10000);s().finishMission(result,reason);
...
s().setPersona('A');s().startMission(0);s().finishMission('fail','difficulty');assert.equal(currentMission(s()).level,1);s().setFailureReason('time');assert.equal(currentMission(s()).level,2);assert.equal(s().fcpsLog.length,1);assert.equal(s().wallet,50000);
